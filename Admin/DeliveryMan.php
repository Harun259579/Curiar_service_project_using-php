...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO deliveryman(name, email, phone, nic, address, gender, password, is_deleted, branch_id)
            VALUES('$name', '$email', '$phone', '$nic', '$address', '$gender', '$password', 0, '$branch_id')";

    if (mysqli_query($con, $sql)) {
        echo "<script>
        alert('Data inserted successfully');
        setTimeout(function() {
            window.location.href='DeliveryMan.php';
        }, 500);
    </script>";
    } else {
                ...
                <div class="col-lg-10">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">DeliverMan List</li>
                        </ol>
                    </nav>
                </div>
                <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == 'admin') : ?>
...
if ($_SESSION['admin'] != 'admin') {
    $getall = getdelivarymanByEmail($_SESSION['admin']); // বা getdeliverymanByEmail()
} else {
    $getall = getAlldeliveryman();
}
...
?>
    ...
    <tr>
        ...
        <td>
            <?php
            $getBranch = getBranchByID($row['branch_id']);
            if ($branchRow = mysqli_fetch_assoc($getBranch)) {
        ...
        <td>
            <a href="deliveryman_edit.php?deliveryman_id=<?php echo $deliveryman_id; ?>" class="btn btn-darkblue">
                <i class="fa-solid fa-edit"></i>
            </a>
            <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == 'admin') : ?>
                <button type="button" onclick="permenantdeleteData(<?php echo $row['deliveryman_id']; ?>,'deliveryman', 'deliveryman_id')" class="btn btn-darkblue">
        ...
        <div class="modal-dialog">
            <div class="modal-content ">
                <div class="modal-header">
                    <h5 class="modal-title text-primary" id="exampleModalLabel">Delivery Man</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="DeliveryMan.php" method="post" id="basicform" data-parsley-validate="" enctype="multipart/form-data">
