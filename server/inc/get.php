...
function getAlldeliveryman()
{
    ...
    $q1 = "SELECT * FROM deliveryman WHERE is_deleted = 0 AND email != 'admin'";
    return mysqli_query($con, $q1);
}


function getdelivarymanByEmail($email)
{
    include 'connection.php';

    $q1 = "SELECT * FROM deliveryman WHERE is_deleted = 0 AND email = '$email'";
    return mysqli_query($con, $q1);
}
...
function getEmployeeTracking() {
    include 'connection.php';

    $query = "
        SELECT r.*, c.name, c.phone, c.address
        FROM request r
        INNER JOIN customer c ON r.customer_id = c.customer_id
        INNER JOIN area a ON a.area_name = r.red_address 
...
function getTrackingByID($track_id) {
  include 'connection.php';
    $sql = "SELECT * FROM request WHERE request_id = '$track_id'";
